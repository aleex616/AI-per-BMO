...
class SunTimeApi(private val context: Context, private val locationManager: LocationManager) {
    ...
    private fun parseSunTimes(response: JSONObject) {
        ...
        fun normalizeTime(time: String): Date? {
            return try {
                ...
                val cal = Calendar.getInstance().apply { setTime(parsed) }
        ...
        if (sunriseTime == null || sunsetTime == null || dawnTime == null || duskTime == null || solarNoonTime == null) {
            ...
            Log.w("SunTimes", "Invalid sun times detected")
            setFallbackTimes()
        }
    }
    ...
    fun setFallbackTimes() {
        ...
        sunriseTime = Calendar.getInstance().apply { time = today.time; set(Calendar.HOUR_OF_DAY, 6); set(Calendar.MINUTE, 0) }.time
        sunsetTime = Calendar.getInstance().apply { time = today.time; set(Calendar.HOUR_OF_DAY, 18); set(Calendar.MINUTE, 0) }.time
        dawnTime = Calendar.getInstance().apply { time = today.time; set(Calendar.HOUR_OF_DAY, 5); set(Calendar.MINUTE, 30) }.time
        duskTime = Calendar.getInstance().apply { time = today.time; set(Calendar.HOUR_OF_DAY, 18); set(Calendar.MINUTE, 30) }.time
        solarNoonTime = Calendar.getInstance().apply { time = today.time; set(Calendar.HOUR_OF_DAY, 12); set(Calendar.MINUTE, 0) }.time
    }
}
