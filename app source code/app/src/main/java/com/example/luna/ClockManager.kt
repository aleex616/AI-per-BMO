...
class ClockManager(
    ...
    private val debugCycleInterval = 5L
    ...
    fun getCurrentTime(): Date = if (isDebugMode) simulatedTime.time else Calendar.getInstance().time

    fun startUpdates() { handler.removeCallbacks(clockUpdateRunnable); handler.post(clockUpdateRunnable) }
    fun stopUpdates() { handler.removeCallbacks(clockUpdateRunnable) }
    ...
    private fun updateClock() {
        val currentTime = if (isDebugMode) {
            ...
            if (simulatedTime.get(Calendar.HOUR_OF_DAY) >= 24) {
                ...
                simulatedTime.add(Calendar.DAY_OF_MONTH, 1); updateSunTimesForSimulatedDay()
            }
            ...
            val sunriseStr = sunTimeApi.sunriseTime?.let { SimpleDateFormat(timePattern, Locale.getDefault()).format(it) } ?: "06:00"
            val sunsetStr = sunTimeApi.sunsetTime?.let { SimpleDateFormat(timePattern, Locale.getDefault()).format(it) } ?: "20:05"
            debugCallback(timeStr, sunriseStr, sunsetStr)
            onTimeChanged(simulatedTime.time)
            simulatedTime.time
        } else {
            val realTime = Calendar.getInstance().time; checkAndUpdateSunTimesForRealTime(realTime); realTime
        }
        val timePattern = fontManager.getTimeFormatPattern().ifBlank { "HH:mm" }
        ...
        if (fontManager.isNightShiftEnabled()) { fontManager.applyNightShiftTransition(currentTime, sunTimeApi, true) }
    }
}
